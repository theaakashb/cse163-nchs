...
class TimeGraph:
    ...
    def time_and_graph(self, fn, max_iters=16):
        print("\n*** Measuring and plotting", fn.__name__, "***\n")
        ns, times = self._do_timing(fn, max_iters)
        self._print_stats(ns, times)
        return self._plot_line(ns, times, fn.__name__)
    ...
    def _format_axes_tightly(self, ax):
        """Use scientific notation + fewer ticks to avoid overlap."""
    ...
    def _plot_line(self, ns, times, method):
        ...
        sns.regplot(x="n", y="time", ci=None, ax=ax2, order=2, color="g", data=df)
        ax1.set_title("Linear")
        ax2.set_title("Quadratic")
        ax1.set_ylabel("Time (seconds)")
        ax2.set_ylabel("")

        self._format_axes_tightly(ax1)
        self._format_axes_tightly(ax2)

        fig.suptitle("Growth of time: " + method)
        plt.show()
        plt.close()
        # return fig
        # plt.savefig("plots/" + method + ".png")